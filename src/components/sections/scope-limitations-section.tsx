...
export function ScopeLimitationsSection() {
  return (
    <section id="limitations" className="py-24 px-6 bg-background border-t border-border relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-20">
          <span className="text-orange-600 dark:text-orange-500 text-[13px] md:text-[14px] font-bold tracking-widest uppercase font-mono block mb-4">{content.limitations.eyebrow}</span>
          <h2 className="text-4xl md:text-5xl font-bold tracking-tight text-foreground mb-6">{content.limitations.heading}</h2>
          <p className="text-muted-foreground max-w-2xl mx-auto text-lg">{content.limitations.subhead}</p>
        </div>
            ...
            <ul className="flex flex-col gap-6 flex-1">
              {content.wontDo.items.map((item, i) => (
                <li key={i} className="flex items-start gap-4">
                  ...
                  <span className="text-[16px] text-muted-foreground leading-relaxed font-medium">{item}</span>
                </li>
              ))}
            </ul>
                ...
                if (i === 0) {
                  ...
                  return (
                    <div key={i} className="flex gap-4 group/item">
                      <div className="w-1.5 h-1.5 rounded-full bg-muted-foreground/30 group-hover/item:bg-amber-500 transition-colors flex-shrink-0 mt-2"></div>
                ...
                if (i === 1) {
                  ...
                  return (
                    <div key={i} className="flex gap-4 group/item">
                      <div className="w-1.5 h-1.5 rounded-full bg-muted-foreground/30 group-hover/item:bg-amber-500 transition-colors flex-shrink-0 mt-2"></div>
                      <p className="text-muted-foreground leading-relaxed text-[15px]">
                        {parts[0]}<span className="text-foreground font-medium bg-muted px-1 rounded mx-1">The lower number is the more honest one.</span>{parts[1] || ''}
                      </p>
                    </div>
                  );
                }
