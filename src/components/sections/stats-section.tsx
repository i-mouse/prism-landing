...
export function StatsSection() {
  return (
    <section className="py-20 px-6 relative border-b border-zinc-800 bg-zinc-950 overflow-hidden">
      <ScatteredRects offset={3.1} />
      {/* Subtle Developer Grid Background */}
      <div className="absolute inset-0 pointer-events-none z-0">
        <div className="absolute inset-0 bg-[linear-gradient(to_right,#ffffff0a_1px,transparent_1px),linear-gradient(to_bottom,#ffffff0a_1px,transparent_1px)] bg-[size:32px_32px] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_50%,#000_20%,transparent_100%)]"></div>
      </div>
      ...
      <div className="max-w-4xl mx-auto text-center relative z-10 flex flex-col items-center">
        ...
        <p className="text-[14px] md:text-[15px] text-zinc-400 font-mono mb-12 max-w-lg mx-auto">
          {content.stats.subtext}
        </p>
        
        <div className="w-full max-w-3xl mx-auto border-t border-zinc-800/60 pt-10 mb-8">
            ...
            {content.stats.metrics.map((metric, i) => {
              ...
              return (
                <div key={i} className="flex flex-col items-center text-center gap-2">
                  ...
                  <span className="text-zinc-500 text-[12px] font-mono leading-relaxed max-w-[180px]">{parts[1]}</span>
                </div>
              );
            })}
          </div>
